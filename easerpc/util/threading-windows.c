...
int os_event_try(os_event_t *event)
{
	...
	if (code == WAIT_TIMEOUT)
		return EAGAIN;	//1. success: the state of this event is nonsignaled.
	else if (code != WAIT_OBJECT_0)
		return EINVAL;	//2. fail

	return 0;	//3. success: (WAIT_OBJECT_0) the state of this event is signaled.
}
